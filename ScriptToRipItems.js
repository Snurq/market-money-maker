...
var tpl = '';
var length = $($0).find('tbody').children().length;

for (var i = 0; i < length; i++) {
	if ((i > 0) && ((i + 1) != length)) {
		tpl += '<Item>\n';
		tpl += "	<id>0</id><name>" + $('.tooltip').eq(i).text().trim() + "</name><price>" + $('.npcvalue').eq(i).text().replace(" gp", "").replace(" ", "") + "</price><buy>0</buy>\n";
		tpl += '</Item>\n';
	}
}

var blob = new Blob([tpl], {type: "application/xhtml+xml;charset=utf-8"});
saveAs(blob, "items.xml");